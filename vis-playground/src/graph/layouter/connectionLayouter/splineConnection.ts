        ...
        if (outgoingConnectionsInside.length > 0 || incomingConnectionsInside.length > 0) {
            ...
            if (nextNode && prevNode) {
                ...
                insideConnections.forEach((connection, index) => {
                    ...
                    const isOutgoing = connection.source == node;
                    const otherNode = isOutgoing ? connection.target : connection.source;
                    const parentRadius = node.parent!.innerCircle.r;
                    const nodeRadius = node.outerCircle.r;
                    const otherNodeRadius = otherNode.outerCircle.r;
                    const sumRadii = nodeRadius + otherNodeRadius;
                    ...
                    const anchor = new Anchor(intersectionPoints[0], slopeVector);
                    const splineControlPointDistanceFactor = 0.3;
                    const sizeArrow = 10;
